    ...
    if($this->UserModel->get($idname,'m_level')>0)
	{
		...
		function post_add_load_type(&$strr,$id_parent,$id_current,$padding)
		{
			...
			for($i=0;$i<$m;$i++)
			{
				...
				$strr.="<div class='item".$temp_add_class."' data-id='".$ar[$i]['id']."' data-title='".$p_t->get($ar[$i]['id'],'m_title')."' onclick='post_write_detail.update_select_type(this)' style='padding-left:".$padding."em'>".$p_t->get($ar[$i]['id'],'m_title')."</div>";
